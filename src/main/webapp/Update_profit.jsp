...
<%
    ...
    try {
        ...
        String selectQuery = "SELECT total_profit FROM users where user_id="+id;
        Statement selectStatement = connection.createStatement();
        resultSet = selectStatement.executeQuery(selectQuery);

        // Update the balance column and set total_invest to 0
        String updateQuery = "UPDATE users SET balance = balance + ? , total_profit = 0 WHERE user_id = ?";
        updateStatement = connection.prepareStatement(updateQuery);

        if (resultSet.next()) {
            // Get values from the result set
            
            BigDecimal totalInvest = resultSet.getBigDecimal("total_profit");
